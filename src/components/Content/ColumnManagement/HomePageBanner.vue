<template>
  <div class="homePageBanner">
	 ...
	 <div class="homePageBanner_table">
		...
		<el-dialog title="添加轮播图" :visible.sync="dialogFormVisible" @close="dialogClose">
			  <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
				  ...
				  <el-form-item label="图片" prop="img">
					 <el-upload
					  ...
					  :action="imgUploadUrlTwo + '/api/Banner/upload'"
...
export default {
  ...
  methods: {
	 ...
	 SelectSearch() {   //下拉框搜索事件
	    ...
	    that.$axios('/api/Banner/getBanner','GET',{page: that.pageNum,rows: that.dataTotalNum, isuse: that.value}).then(function(res){
		    that.tableData = res.data.items;
			that.total = res.data.total;
			loading.close();
		}).catch(function(err){
	     	loading.close();
		    console.log('首页轮播图下拉框选项数据状态时加载失败: ' + err);
			that.$message.error('首页轮播图下拉框选项数据状态时加载失败');
		});
	 },
	 editBannerTaber(row) {  //banner表格编辑事件
	     var that = this;
         
			 that.ruleForm = JSON.parse(JSON.stringify(row));   //克隆对象
			 that.dialogFormVisible = true;
	 },
	 removeBannerTaber(row) {  //banner表格删除事件
	     var that = this;
		     that.$confirm('此操作将永久删除, 是否继续?', '提示', {
                  ...
                  that.$axios('/api/Banner/removBanner','GET',{id: row.id}).then(function(res){
						that.reload();
						}).catch(function(err){
						console.log('首页轮播图banner表格删除时失败: ' + err);
						that.$message.error('首页轮播图banner表格删除时失败');
					 });				   
	 ...
	 handlePageAxiosRequest(){     //分页请求数据
	    ...
	    that.$axios('/api/Banner/getBanner','GET',{page: that.pageNum,rows: that.dataTotalNum,isuse: that.value}).then(function(res){
		    ...
		    that.pageFlag = true;
	     	loading.close();
		    console.log(err);
			that.$message.error('首页轮播图分页获取时失败');
		});
	 },
     //  表单方法
	   ...
	   var that = this;
		   ...
		   that.$refs[formName].validate((valid) => {
			  if (valid) {
				...
				that.$axios('/api/Banner/addBanner','POST',that.ruleForm).then(function(res){
					that.reload();
					}).catch(function(err){
					console.log('首页轮播图banner表格编辑时失败: ' + err);
					that.$message.error('首页轮播图banner表格编辑时失败');
				});
  ...
  created: function () {
	    ...
	    that.$axios('/api/Banner/getBanner','GET',{page: that.pageNum,rows: that.dataTotalNum}).then(function(res){
		    that.tableData = res.data.items;
			that.total = res.data.total;
			loading.close();
		}).catch(function(err){
	     	loading.close();
		    console.log('首页轮播图初始化加载失败: ' + err);
			that.$message.error('首页轮播图初始化加载失败');
		});
